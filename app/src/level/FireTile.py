...
class FireTile(Tile):
    """FireTile class is full for ground enemies and deals damage to player.
    """
    def __init__(
        self,
        pos: Tuple[int, int],
        ...
        self.color = (100, 40, 255)

        CFG_SCALE = config['scale']
        self.drawable = [
            sh.Rectangle(
                self.pos[0] * self.size[0] * CFG_SCALE,
                self.pos[1] * self.size[1] * CFG_SCALE,
                self.size[0] * CFG_SCALE, self.size[1] * CFG_SCALE, self.color,
                batch=debug_batch
            ),
            sh.Circle(
    ...
    def recieved_damage(self) -> int:
        """Amount of damage players gets, config value for empty tile."""
        return config['fire_tile_dmg']
